...
import net.babys_care.app.api.requests.BrowseHistoryRequest
...
class BrowsingHistoryViewModel: BaseViewModel() {
    ...
    val browsingHistories: MutableList<BrowseHistory> = mutableListOf()
    ...
    fun fetchBrowsingHistory(apiToken: String) {
        viewModelScope.launch {
            try {
                val response = ApiClientManager.babyCareApi.getBrowsingHistory(
                    BrowseHistoryRequest(apiToken)
                )
                if (response.isSuccessful) {
                    response.body()?.data?.browse_histories?.let { list ->
                        browsingHistories.addAll(list)
                    }
                } else {
                    debugLogInfo("Error: ${response.errorBody()}")
                }
            } catch (ex: java.lang.Exception) {
                debugLogInfo("Exception: $ex")
            }
